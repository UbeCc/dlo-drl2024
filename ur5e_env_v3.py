#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time         : 2024/5/28 14:39
# @Author       : Wang Song
# @File         : ur5e_env_v3.py
# @Software     : PyCharm
# @Description  : 3D cable manipulation environment with UR5e robot
import time
import os
import numpy as np
...
from manipulator_mujoco.robots import Cable_test, Cable_3d
...
class UR5eEnv_v3(gym.Env):
    ...
    def __init__(self, render_mode=None):
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(42, 3), dtype=np.float64)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        observation = np.concatenate([observation[:, 0:3], target_pos])
        return observation
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        target_seed = np.random.randint(0, 166)
        self.target_pos = self.generate_target_pos(seed=target_seed)
        self.current_pos.append(self.target_pos)
        # reset physics
        with self._physics.reset_context():
            # 能够保证开始时即抓取成功的机械臂的初始关节角度
            self._physics.bind(self._arm.joints).qpos = [-0.27311595, -1.5970241,   1.61356987, -1.59329002, -1.56208292, -0.27413166]
            self._physics.bind(self._arm2.joints).qpos = [-0.26607908, -1.59599941,  1.61440437, -1.59531655, -1.57980539, -0.26392371]
            ...
            # 生成随机位置的目标
            for i in range(self.target_num):
                self._targets[str(i)].set_mocap_pose(self._physics,
                                                     position=[self.target_pos[i][0], self.target_pos[i][1], self.target_pos[i][2]],
                                                     quaternion=[0, 0, 0, 1])
    ...
    def step(self, action: np.ndarray) -> tuple:
        ...
        target_pose1[2] = current_pose1[2] + action1[2]
        target_pose2[2] = current_pose2[2] + action2[2]
        target_pose1[3] = 0.0
        target_pose2[3] = 0.0
        target_pose1[4] = 0.0
        target_pose2[4] = 0.0
        ...
        self.control_steps = 0
        # 如果目标位置不合理，则直接失败，退出
        distance = np.linalg.norm(target_pose1[[0, 1, 2]] - target_pose2[[0, 1, 2]])
        if distance > 0.53:
        ...
        while (time.time() - time_control_start < 1 / self.control_frequency):
            ...
            error1 = np.linalg.norm(target_pose1[[0, 1, 2]] - current_pose1[[0, 1, 2]])
            error2 = np.linalg.norm(target_pose2[[0, 1, 2]] - current_pose2[[0, 1, 2]])
        ...
        # ********************action penalty********************
        warning = np.linalg.norm(
            current_pose1[[0, 1, 2]] - current_pose2[[0, 1, 2]]) > 0.45  # punish the stretching of the cable
        # ******************termination********************
        current_tcp_left = observation[-1]
        current_tcp_right = observation[-2]
        if self.steps <= 1 and dlo_error < 0.01:
            print('initial error too small.')
            truncated = True
        if current_tcp_left[1] < -0.1 or current_tcp_left[1] > 0.30 or current_tcp_left[0] < 0.35 or current_tcp_left[
            0] > 0.75:
            print('left arm out of range.')
            distance_tag = True
            truncated = True
        if current_tcp_right[1] < 0.20 or current_tcp_right[1] > 0.51 or current_tcp_right[0] < 0.35 or \
                current_tcp_right[
                    0] > 0.75:
        ...
        # ********************reward********************
        # done = dlo_error < 0.01
        # reward = self.calculate_reward(dlo_error, weighted_similarities, distance_tag, warning, done)
        ...
        # vector = observation1[0:42] - self.current_pos[-1]
        # error = np.sqrt(np.sum(vector * vector) / self.target_num)
        # if error > 0.005:
        #     self.current_pos.append(observation1[0:42])
        # if truncated == True:
        #     print(len(self.current_pos))
        #     with open('current_pos.txt', 'w') as f:
        #         for i in range(len(self.current_pos)):
        #             f.write(str(self.current_pos[i].tolist()) + '\n')
        #     print([subarray.tolist() for subarray in self.current_pos])
        # print("time_cost:", time.time() - time_control_start)
    ...
    def calculate_reward(self, dlo_error, weighted_similarities, distance_tag, warning,done):
        ...
        # 平滑的相似度奖励
        smooth_similarity_reward = 0.15 * weighted_similarities

        # 其他奖励项
        penalty_distance = -20 * distance_tag
        penalty_warning = -1 * warning
