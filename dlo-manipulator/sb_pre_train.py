#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time         : 2024/5/18 17:04
# @Author       : Wang Song
# @File         : sb_pre_train.py
# @Software     : PyCharm
# @Description  :
import os
import random
...
load_model = DDPG.load("./logs/best_model_ddpg.zip", env=env, action_noise=action_noise, verbose=1,learning_starts=100,
                       tensorboard_log="./DDPG_tensorboard/")
...
# Train the agent
timesteps = 200000
load_model.learn(total_timesteps=int(timesteps), callback=callback, tb_log_name="run_env_v32_3", log_interval=1)

# After training, evaluate the agent
mean_reward, std_reward = evaluate_policy(load_model, env, n_eval_episodes=10, render=True)
