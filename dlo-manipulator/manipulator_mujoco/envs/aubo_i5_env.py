...
class AuboI5Env(gym.Env):
    ...
    def __init__(self, render_mode=None):
        """
初始化环境，设置观察空间和动作空间。
创建仿真场景的组件，包括 AuboI5 机械臂、AG95 夹具、目标 Target 和一个立方体 Primitive。
将机械臂和立方体附加到 StandardArena 上。
生成 MJCF 模型并初始化物理引擎 Physics。
设置操作空间控制器 OperationalSpaceController。
        """
        ...
        '''
        assert 语句的目的是在环境初始化时确保 render_mode 参数是有效的。
        如果传入的 render_mode 不是 None 并且也不在支持的渲染模式列表中，assert 语句将会引发一个 AssertionError 异常。
        这样可以防止环境在不支持的渲染模式下运行，确保环境的稳定性和预期行为。'''
        ...
        # mocap target that OSC will try to follow在模拟环境中创建一个运动捕捉目标，供操作空间控制器追踪。
    ...
    def _render_frame(self) -> None:
        ...
        if self._render_mode == "human":
            ...
            if time_until_next_step > 0:
                time.sleep(time_until_next_step) # 这个地方好像很影响效果
